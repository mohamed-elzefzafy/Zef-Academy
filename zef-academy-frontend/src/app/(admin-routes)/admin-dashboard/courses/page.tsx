...
const InstructorCoursesPage = () => {
  ...
  const { data, isLoading ,refetch} = useGetAdminDashboardCoursesQuery({
    page: currentPage,
    limit: pageSize,
  });
  const [deleteCourseAdminInstructorPage] =
    useDeleteCourseAdminInstructorPageMutation();
    const [updateCourseToPublish] = useUpdateCourseToPublishMutation();
  ...
  const columns: GridColDef[] = [
    ...
    },
    {
      ...
      renderCell: (params: GridRenderCellParams) => (
        ...
        >
      {params.row.isPublished === "Published" ? "Published" :   (    <IconButton
            onClick={() =>
              onPublishCourse(params.row.courseId)
            }
  ...
  const onDeleteCourse = async ({
    ...
    const onPublishCourse = async (courseId : string) => {
    try {
      const willPublish = await swal({
        title: "Are you sure?",
        text: "If you publish this course, the instructor an add lectures, without your accept",
        icon: "warning",
        dangerMode: true,
      });

      if (willPublish) {
        await updateCourseToPublish(courseId).unwrap();
        router.refresh();
        refetch();
