...
import {  useGetInstructorCountsQuery } from "@/redux/slices/api/utilsApiSlice";
import { useAppSelector } from "@/redux/hooks";

const InstructorDashboardPaper = () => {
    const { userInfo } = useAppSelector((state) => state?.auth);
  const { data: counts } = useGetInstructorCountsQuery(userInfo._id);
  ...
  const data = [
    ...
    {
      label: "Lectures",
      value: counts?.lecturesCount,
      icon: <LocalLibrary fontSize="large" color="primary" />,
      path: "/instructor-dashboard/lectures",
    },
  ];
...
export default InstructorDashboardPaper;
