...
async create(
  ...
  const course = await this.courseService.findOneWithoutpopulate(createLectureDto.course);

  if (!course.isPublished) {
    throw new BadRequestException('The course must be published before uploading lectures');
  }
  ...
  async updateLecturePosition(id: string, newPosition: number, user: JwtPayloadType) {
  const lecture = await this.findOne(id, user);
  if (!lecture) {
    throw new NotFoundException(`Lecture with id ${id} not found`);
  }
  ...
  const clampedPosition = Math.max(0, Math.min(newPosition, remainingLectures.length));
...
async remove(id: string, user: JwtPayloadType) {
  ...
  return { message: `Lecture with id (${id}) was removed and positions updated.` };
}
